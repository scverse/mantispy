...
def chatterjee_xi(x: np.ndarray, y: np.ndarray, m: int = 1, seed: int = 0, min_finite: int = 40) -> np.ndarray:
    ...
    Args:
        ...
        m: Right nearest neighbors, as in :cite:t:`Lin_2022`. ``m=1`` is Chatterjee's coefficient :cite:p:`Chatterjee_2020` up to a term of order ``1/n``. Larger ``m`` has the same limit under dependence and a lower noise floor under independence, so a fixed threshold is more reliable.
...
def feature_select_chatterjee(
    ...
    Args:
        ...
        m: Right nearest neighbors :cite:p:`Lin_2022`. ``m=1`` is Chatterjee's coefficient :cite:p:`Chatterjee_2020` up to a term of order ``1/n``; larger values lower the noise floor without changing what the statistic converges to.
