...
def outliers(
    ...
    Every method produces a score where higher means more outlying, and the same
    thresholding applies to all of them, so ``contamination`` is the flagged fraction
    whichever method is used.

    Args:
        adata: Object to flag.
        method: ``"ecod"`` is parameter-free and interpretable per feature,
        ...
        key: Restrict to features flagged by this boolean ``var`` column, usually
            ``"selected"``. Falls back to every feature when the column is absent.
        by: Threshold within each group of this ``obs`` column, e.g. per plate, rather than
            globally.
