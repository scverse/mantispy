...
The package logger has no handler of its own, so diagnostics such as "dropped 2
feature(s)" or "dropped 960 of 960 cells" would not be shown. This module installs a
handler and sets its level from ``verbosity``, on the same 0-3 scale as scanpy.

The class comes from scverse-misc, so every setting is also read from a ``MANTISPY_``
environment variable and can be changed for a block with ``settings.override``.
"""
...
class _CurrentStderr:
    ...
    Jupyter, ``contextlib.redirect_stderr``, pytest and captured subprocesses replace
    ``sys.stderr`` later, and a bound handler would keep writing to the replaced stream.
    """
...
# Without this, calling logging.basicConfig(), as notebook tutorials often do, prints
# every mantispy line twice.
