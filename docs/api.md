...
behind the profiles into `SpatialData`, from a Cell Painting Gallery {cite:p}`Weisbart_2024` source or an `ExportForSpatialData` plate
...
Seurat RPCA it ranked in the top three in every scenario of {cite:t}`Arevalo_2024`. It needs the
optional extra:
