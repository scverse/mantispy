...
- `mantispy.io`: `stamp` supplies the annotation columns `var` does not carry before it records the schema version rather than after, so stamping a view no longer discards the record in the act of writing `var`, and a well-level subset stops reading back as single-cell. The fill stays in `io.stamp`, the entry point for an object built elsewhere, and out of the stamp every tool calls, which would otherwise repair an annotation its input had damaged and leave `io.write` nothing to report
