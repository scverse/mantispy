...
from _testdata import BATCH, CELL_PAINTING_CHANNELS, EXPORT_OBJECTS, EXPORT_SHAPE, FIELDS, OVERLAY_PLATE, PLATE, PREFIX
...
def test_a_gallery_plate_reads_whole(sdata: sd.SpatialData) -> None:
    ...
    # only well A01 was analysed, so only it has labels and rows in the cell table
    ...
    assert wells.n_obs == len(sdata.shapes[f"{PLATE}_wells"]) == 384
    # the URL_Illum* column names a correction image, not a channel
    image = sdata[f"{PLATE}_A01_s1_image"]["scale0"]["image"]
    assert list(image.coords["c"].to_numpy()) == list(CELL_PAINTING_CHANNELS)
...
def test_export_rows_join_onto_the_label_arrays(export: Path) -> None:
    ...
    assert set(table.uns["spatialdata_attrs"]["region"]) == {f"{field}__Cells" for field in FIELDS}
...
def test_a_failed_field_is_left_out_along_with_its_rows(make_export) -> None:
    ...
    assert sdata.tables["cells"].n_obs == 2
    assert set(sdata.tables["cells"].uns["spatialdata_attrs"]["region"]) <= set(sdata.labels)
